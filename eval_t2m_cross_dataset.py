...
from motion_loaders.dataset_motion_loader import get_dataset_motion_loader_for_crossdata
...
def load_ddim_models(model_opt, which_model):
    ...
    print("[Info] using ddim steps: ", model_opt.ddim_steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--evaluator', type=str, default='text_mot_match')
    parser.add_argument('--cal_mm', action='store_true', help='calculate multimodality')
    parser.add_argument('--keyframe_interval', type=int, default=5, help='keyframe interval (in embeddings)')
    ...
    out_dir = pjoin(root_dir, f'eval_{opt.inference_setting}' 
    + f'_KI{opt.keyframe_interval}' if opt.inference_setting == 'keyframe' else ''
    + f'_{dataset_name}', opt.which_model_for_eval)
    ...
    dataset_opt_path = f'./configs/dataset_opt/kit_data_opt.txt' if opt.dataset_name == 'kit' \
        else f'./configs/dataset_opt/t2m_data_opt.txt'
    
    wrapper_opt = get_opt(dataset_opt_path, torch.device('cuda'))
    wrapper_opt.evaluator = opt.evaluator
    ...
    eval_val_loader, _ = get_dataset_motion_loader_for_crossdata(dataset_opt_path, 32, 'test', device=opt.device, data_root=opt.data_dir)
    ...
    f.write("[Info] Using DDIM steps: %s\n" % opt.ddim_steps)

    print('loading checkpoint {}'.format(file))
    latent_transformer, diff_model, diffusion = load_ddim_models(opt, file)
    ...
    # print("[Warning] Hardcoded test set!")
    fid = []
    div = []
    top1 = []
    top2 = []
        ...
        try:
            with torch.no_grad():
                best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = eval_t2m.evaluation_latent_transformer(
                    ...
                    diffusion=diffusion, diff_model=diff_model, cond_scale=opt.guidance_param,
                    infer_timesteps=opt.trans_infer_timesteps, progress=False, inference_setting=opt.inference_setting, cal_mm=opt.cal_mm, keyframe_interval=opt.keyframe_interval
                )
            fid.append(best_fid)
            div.append(best_div)
