...
class joints2smpl:
    ...
    def npy2smpl(self, npy_path):
        ...
        for sample_i in tqdm.tqdm((range(n_samples))):
            print(f"motions.shape= {motions['motion'][sample_i].shape}")
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        # print(f"input_joints: {input_joints}")
        # print(f"input_joints.shape: {input_joints.shape}")  
        ...
        if init_params is None:
            ...
            # print(f"pred_betas.shape: {pred_betas.shape}")
            # print(f"pred_cam_t.shape: {pred_cam_t.shape}")
        else:
            # print("init_params is not None")
        ...
        # print(f"new_opt_pose.shape_init: {new_opt_pose.shape}")
        thetas = new_opt_pose.reshape(self.batch_size, 24, 3)
        thetas_6d = geometry.matrix_to_rotation_6d(geometry.axis_angle_to_matrix(thetas))  # [bs, 24, 6]
        global_orient = thetas[:, 0]
        thetas = thetas[:, 1:23]
        # print(f"thetas.shape: {thetas.shape}")
        thetas = thetas.unsqueeze(0).permute(0, 2, 3, 1)  # [1, 120, 3, 22]

        root_loc = torch.tensor(keypoints_3d[:, 0])  # [bs, 3]
        root_loc_6d = torch.cat([root_loc, torch.zeros_like(root_loc)], dim=-1).unsqueeze(1)  # [bs, 1, 6]
        root_loc = root_loc.T
        thetas_6d = torch.cat([thetas_6d, root_loc_6d], dim=1).unsqueeze(0).permute(0, 2, 3, 1)  # [1, 25, 6, 196]

        return thetas.clone().detach(), global_orient, root_loc, {'pose': new_opt_joints[0, :24].flatten().clone().detach(), 'betas': new_opt_betas.clone().detach(), 'cam': new_opt_cam_t.clone().detach()}, thetas_6d
