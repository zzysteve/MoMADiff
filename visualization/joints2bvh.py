...
# skeleton = Skeleton(paramUtil.t2m_kinematic_chain, "cpu")
...
if __name__ == "__main__":
    ...
    folder = '/gpfsdata/home/kongbohan/Motion_Generation/momask/generation/tmp_2/a_person_kicks_two_times_on_his_left_then_kicks_forward_two_times'
    files = os.listdir(folder)
    files = [f for f in os.listdir(folder) if f.endswith('.npy')]
    # files = [f for f in files if 'repeat' in f]
    converter = Joint2BVHConvertor()
    for f in tqdm(files):
        joints = np.load(os.path.join(folder, f), allow_pickle=True)
        out_file = os.path.join(folder, f'ik_{f}'.replace('npy', 'bvh'))

        # diagnostics (run for one file)
        anim, glb = converter.convert(joints, out_file, foot_ik=True)  # glb 已在 convert 中返回
        # print shapes and some stats
        # print("anim.rotations.shape:", getattr(anim, 'rotations').qs.shape if hasattr(anim, 'rotations') else None)
        # print("glb.shape:", glb.shape)  # expect (N, J, 3)
        # print("glb min/max per axis:", glb.min(axis=(0,1)), glb.max(axis=(0,1)))
        # print("first frame joints (0..10):\n", glb[0, :11, :])
        # print("kinematic_tree length:", len(paramUtil.t2m_kinematic_chain))
        # # check joint count consistency
        
        # assert glb.shape[1] == sum(len(chain) for chain in paramUtil.t2m_kinematic_chain), \
        #     f"joint count mismatch: glb has {glb.shape[1]}, kinematic tree expects {sum(len(c) for c in paramUtil.t2m_kinematic_chain)}"

        anim, glb = converter.convert(joints, out_file, foot_ik=True)
        out_mp4 = out_file.replace(".bvh", ".mp4")
        positions = Animation.positions_global(anim)  # shape (N, J, 3)
        plot_3d_motion(out_mp4, my_kinematic_chain, positions, title=f)
        # plot_3d_motion(out_mp4, paramUtil.t2m_kinematic_chain, positions, title=f)
